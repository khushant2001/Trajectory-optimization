...
def toy_car_obstacle_avoidance():
        # Define the dynamics!!!
    # x_dot = vcos(theta)
    # y_dot = vsin(theta)
    # theta_dot = omega
    ...
    Q = DM([
        [100,0,0],   # Increased weight on x and y for faster convergence
        [0,100,0],
        [0,0,.1]
    ])
    ...
    for i in range(3*(1+horizon_steps),(n_states+len(obs_x))*(1+horizon_steps)):
        lbg[i] = -inf
    ...
    while norm_2(x0 - xf) > 0.01 and t0 < 20:
        ...
        # Draw the obstacle
        for xi, yi, ri in zip(obs_x, obs_y, r0):
            plt.gca().add_patch(plt.Circle((xi, yi), ri, color='y', alpha=0.5,label = 'Obstacle'))
